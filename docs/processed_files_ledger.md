...
| `jobs/` | historically_active | Real package split exists in sandbox; checkpointed and later audit-validated as a real package lane, not empty scaffolding | high | current sandbox working tree + direct package listing + wrapper reads + post-checkpoint audit | Some modules still depend on shared/deferred infra such as `scripts.atomic_csv` |
...
| `scripts/normalize_conditions.py` | historically_active | Structural-refactor lane: compatibility shim into jobs package | medium | current sandbox working tree + prior direct file read + diff stat | Wrapper-style forwarding script |
| `scripts/normalize_listing_csvs.py` | historically_active | Structural-refactor lane: compatibility shim into jobs package | medium | current sandbox working tree + direct file read + diff stat | Wrapper-style forwarding script |
