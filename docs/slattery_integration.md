...
This work is a local source change on `codex/slattery-integration-20260911`, based on `origin/main` at `ac1b2b5`. Production was read-only: its observed deployed commit was `ac1b2b5` and its latest daily run had succeeded. Push, governed deployment and a production refresh have not occurred in this work slice. Governed schemas, resale curves and native Grays extraction remain unchanged.
