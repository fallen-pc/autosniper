...
- `toyota_corolla_ascent-sport_zre182r_hatch_auto_petrol` now has a manual/provisional shape from Carsales-style Ascent Sport evidence, but the repo still shows `0` active Autotrader rows and only `4` sold Grays rows for the mapped family, so completeness is not yet proven.
