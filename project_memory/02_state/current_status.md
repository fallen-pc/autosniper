...
- Hourly active snapshots are now treated as short-lived monitoring data: the live `active_snapshots.csv` should keep only the latest row per active URL, and older/inactive hourly rows should live in the archive path for optional later analysis and end-of-day commits.
