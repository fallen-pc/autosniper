...
- The current Buy/Avoid/Review policy has been measured against outcome evidence for the first time (2026-08-19, session dc169735). No cars have been bought, so `actual_profit` is empty everywhere it was expected; instead, `scripts/build_retail_exit_ledger.py` verifies Autotrader listing removal by polling each listing's own URL for `removed=true` rather than trusting scrape-absence, and `scripts/build_replay_outcomes.py` joins that against historical Grays sold rows and replays the current policy. Result: 989 rows, precision 100%, recall 12.5%, published on the Model Proof page. Recall is capped less by the price anchor than by two things: repair-cost gating (see below) and the interstate-buying block, which is a deliberate operating constraint, not a bug.

- Repair pricing coverage is now measured, not assumed, and one structural bug behind it is fixed. `scripts/build_repair_pricing_matrix.py` scanned all 20,406 sold listings and found 252 of 290 (canonical repair type x vehicle class) cells with no schedule price — 22,902 unpriced listing-hits, affecting 48.8% of sold listings. Root cause: `shared/repair_pricing_schedule.py`'s quote-drafting logic was class-blind — every quote request defaulted to `small_hatch`, and a canonical was dropped from "needs pricing" the moment any one class got priced, so cosmetic/paint/seat/trim/corrosion pricing never asked about SUV, ute, van, or sedan. Fixed with class-aware logic (`missing_vehicle_classes_for`, `_class_priority_order`). 20 real quote requests were sent 2026-08-21 to proven suppliers for the highest-impact gaps (RQ-0059 through RQ-0078); 4 interim rows were added from class-specific web research, marked `confidence=low` so they are never mistaken for a real quote. The two single biggest gaps, `cosmetic_surface_damage` and `paint_damage`, have no real per-class pricing evidence anywhere and depend entirely on the sent quotes landing. See `project_memory/02_state/quote_pipeline_class_blindness_fix_2026-08-19.md` and `repair_quote_batch_and_research_2026-08-19.md`.

- Rule to hold going forward on any per-class or per-segment pricing gap: never derive one class's price by scaling another class's number with a guessed multiplier. `shared/repair_pricing_schedule.py`'s own schedule validator already enforces this ("evidence_source is required; do not invent class multipliers"). Source independent evidence for the specific class, or leave the cell unpriced.
