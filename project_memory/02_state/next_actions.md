...
22. The next coverage-expansion step after the new tag batch is curves, not more generic tag sprawl. Camry Altise `ASV50R` and Mazda CX-5 Maxx Sport diesel `KE` are now saved; next priority order is Hyundai ix35 `LM`, then Hyundai Getz `TB`.
