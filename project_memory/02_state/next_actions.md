...
20. Continue the profit/valuation audit from the next remaining decision surfaces after the conservative-margin, top-buy, bid-basis, and max-bid-safety split fixes: max-bid recommendation consistency in the curve path, AI Analysis max-bid display/filter correctness, and any stale stored valuation rows that still need recomputation.
