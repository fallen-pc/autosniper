...
20. Continue the profit/valuation audit from the next remaining decision surfaces after the conservative-margin, top-buy, bid-basis, max-bid-safety split, AI max-bid display cleanup, legacy `expected_profit` basis fix, and dashboard/outcome consumer cleanup: revalue stale stored rows so old semantics do not linger in `ai_listing_valuations.csv`, then check any remaining pages still reading legacy profit fields directly.
