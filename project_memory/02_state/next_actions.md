...
20. Continue the profit/valuation audit from the next remaining decision surfaces after the conservative-margin, top-buy, bid-basis, max-bid-safety split, AI max-bid display cleanup, and legacy `expected_profit` basis fix: check whether dashboard/outcome surfaces should stop reading overloaded legacy fields directly, and revalue stale stored rows so old semantics do not linger in `ai_listing_valuations.csv`.
