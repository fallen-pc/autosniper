...
20. Continue the profit/valuation audit from the next remaining decision surfaces after the conservative-margin, top-buy, bid-basis, max-bid-safety split, AI max-bid display cleanup, legacy `expected_profit` basis fix, dashboard/outcome consumer cleanup, and Radar cleanup: sanity-check the live pages against real listings, then remove or quarantine any remaining legacy profit compatibility fields only when downstream consumers are proven clean.
