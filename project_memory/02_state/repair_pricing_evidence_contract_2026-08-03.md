...
- `CSV_data/reports/repair_pricing_schedule.csv` is the governed evidence baseline required by fresh checkouts. Although `CSV_data/**` is normally ignored as runtime data, this file must be force-added deliberately whenever an approved evidence row changes.
