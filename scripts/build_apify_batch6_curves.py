"""Build the sixth governed curve batch from accumulated exact Carsales evidence."""
...
LANES = [
    dict(
        key="ford_kuga_ambiente_tf",
        make="ford", model="kuga", retail=r"\bAmbiente\b.*\bTF\b.*\bAuto\b.*\bAWD\b",
        sold=r"^awd ambiente tf$", years=(2013, 2014), anchors=[2013, 2014],
        body="wagon", body_aliases="wagon|suv", transmission="auto",
        sold_transmission="auto", fuel="petrol", engine=r"1\.6L",
        badge="awd ambiente", aliases="awd ambiente|ambiente|awd ambiente tf", series="tf",
        excluded="manual|diesel|hybrid|mkii|mk ii|trend|titanium|2wd",
        base="ford_kuga_awd-ambiente_tf_wagon_auto_petrol",
        match="ford_kuga_awd-ambiente_petrol_auto_wagon_tf",
    ),
    dict(
        key="holden_barina_tk_auto",
        make="holden", model="barina", retail=r"\bTK\b.*\bAuto\b",
        sold=r"^tk$", years=(2006, 2011), anchors=[2006, 2009, 2011],
        body="hatch", body_aliases="hatch|hatchback", transmission="auto",
        sold_transmission="auto", fuel="petrol", engine=r"1\.6L",
        badge="tk", aliases="tk", series="tk",
        excluded="manual|diesel|hybrid|sedan|tm|xc|spark|cd|cdx",
        base="holden_barina_tk_hatch_auto_petrol",
        match="holden_barina_tk_petrol_auto_hatch",
    ),
    dict(
        key="volkswagen_golf_103tsi_highline_a7",
        make="volkswagen", model="golf", retail=r"\b103TSI Highline 7\b.*\bAuto\b",
        sold=r"^103tsi highline a7$", years=(2012, 2015), anchors=[2012, 2013, 2015],
        body="hatch", body_aliases="hatch|hatchback", transmission="auto",
        sold_transmission="auto", fuel="petrol", engine=r"1\.4L",
        badge="103tsi highline", aliases="103tsi highline|103tsi highline a7|103tsi highline 7",
        series="a7", excluded="manual|diesel|hybrid|wagon|gti|golf r|110tsi|110tdi|a6|vi|6",
        base="volkswagen_golf_103tsi-highline_a7_hatch_auto_petrol",
        match="volkswagen_golf_103tsi-highline_petrol_auto_hatch_a7",
    ),
]
...
def main() -> int:
    ...
    for lane in LANES:
        ...
        proposal, metadata = propose_curve_from_evidence(
            ...
            evidence_source="accumulated exact private Carsales Apify evidence through batch 5",
        )
        ...
        allowed.append({
            ...
            "body": lane["body"], "fuel": lane["fuel"], "transmission": "auto",
    ...
    snapshot_curve_version(
        curves_path, source="codex_grays_apify_batch6",
        change_summary=(
            "Added Ford Kuga Ambiente TF AWD petrol, Holden Barina TK automatic hatch, and "
            "Volkswagen Golf 103TSI Highline A7 curves from exact private Carsales evidence "
            "backed by live Grays demand"
        ),
    )
    return 0
